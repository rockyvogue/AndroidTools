...
public class RomCustomMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(CATION_CUSTOM_UPDATE)) {// 添加蓝牙电话传输过来的电话到数据库
			...
			LOG.writeMsg(this, LOG.MODE_VOICE, "CATION_CUSTOM_UPDATE flag:" + flag);
			switch (flag) {
			case CONTACT_SYNC:
	...
	private void resetContact() {
		LOG.writeMsg(this, LOG.MODE_VOICE, "resetContact");
	...
	private void resetPhone() {
		LOG.writeMsg(this, LOG.MODE_VOICE, "resetPhone");
		ContentResolver resolver = mContext.getContentResolver();
		Uri uri = Uri
...
//		LOG.writeMsg(this, LOG.MODE_VOICE, "name: " + name + " number: " + number);
		ContentResolver resolver = mContext.getContentResolver();
		Uri uri = Uri
				.parse("content://cn.yunzhisheng.vui.provider.ContactProvider/contact");
...
//					LOG.writeMsg(this, LOG.MODE_VOICE, " contact_id:" + contact_id);
			...
			if (contactdIds.size() > 0) {
				for (long id : contactdIds) {
					if (deletePhone(id, number)) {
						...
						LOG.writeMsg(this, LOG.MODE_VOICE, CATION_CUSTOM_UPDATE + " deleted");
					}
				}
			}
		}
	...
	private void insertContactInfosInNewThread(final ArrayList<String> contacts) {
		LOG.writeMsg(this, LOG.MODE_VOICE, "insertContactInfosInNewThread");
		if (mThread == null) {
				...
				public void run() {
					if (contacts != null && contacts.size() > 1) {
						...
						for (int i = 0; i < contacts.size(); i++) {
							...
							LOG.writeMsg(this, LOG.MODE_VOICE, "info : " + Arrays.asList(info));
							if (info != null && info.length == 2) {
								insertContact(info[0], info[1]);
							}
						}
					}
					LOG.writeMsg(this, LOG.MODE_VOICE, CATION_CUSTOM_UPDATE + " sync end");
					mThread = null;
				}
			});
		}
		if (!mThread.isAlive()) {
			LOG.writeMsg(this, LOG.MODE_VOICE, CATION_CUSTOM_UPDATE + " start sync");
			mThread.start();
		}
	}
}
